...
```mermaid
flowchart LR
    N0["主播端 (Offerer) 观众端 (Answerer)"]
    N1["1. CreateOffer() 生成 SDP Offer SDP Offer 包含: 支持的所有编解码 所有 ICE 候选 DTLS 指纹 SDP Answer 包含: 选定的编解码 对方的 ICE 候选 对方的 DTLS 指纹 3. SetLocal/RemoteDescription 双方配置生效"]
    N2["2. CreateAnswer() 选择合适的配置 生成 SDP Answer"]
...
flowchart LR
    N0["主播端 观众端"]
    N1["SDP Offer (无候选) Candidate: host 192.168.1.2 Candidate: srflx 1.2.3.4 Candidate: relay 5.6.7.8"]
...
flowchart LR
    N0["主播端 观众端"]
    N1["1. ClientHello - 支持的加密套件 - 随机数 5. ClientKeyExchange - 预主密钥 (用服务器公钥加密) 6. ChangeCipherSpec 7. Finished [加密通道建立] 导出 SRTP 密钥"]
    N2["2. ServerHello - 选择的加密套件 - 随机数 3. Certificate - 服务器证书 4. ServerHelloDone 8. ChangeCipherSpec 9. Finished [加密通道建立] 导出 SRTP 密钥"]
...
```mermaid
flowchart TB
    N0["DTLS 握手完成 ▼ 导出密钥材料 (Exporter) > SRTP 加密密钥 > SRTP 认证密钥 > SRTP Salt"]
...
```mermaid
flowchart TB
    N0["应用层数据 ▼ SCTP (数据分片、流控制、拥塞控制) DTLS (加密) UDP"]
