...
```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ MP4文件 │     │ H.264   │     │ YUV像素 │     │ 屏幕   │
│ 100MB   │ ──→ │ 压缩数据 │ ──→ │ 原始    │ ──→ │ 显示   │
└─────────┘     └─────────┘     └─────────┘     └─────────┘
     │                │                │                │
     ↓                ↓                ↓                ↓
  解封装           解码            颜色转换          渲染
  Demuxer        Decoder          (自动)          Renderer
```
...
┌─────────────────────────────────────────┐
│ YYYYYYYY YYYYYYYY YYYYYYYY YYYYYYYY    │  1920×1080 = 2,073,600 字节
│ YYYYYYYY YYYYYYYY YYYYYYYY YYYYYYYY    │  每个像素1字节
│ ... (1080行)                            │
├─────────────────────────────────────────┤
│ UUUU UUUU UUUU UUUU                    │  960×540 = 518,400 字节
│ ... (540行)                             │  每4个Y共享1个U
├─────────────────────────────────────────┤
│ VVVV VVVV VVVV VVVV                    │  960×540 = 518,400 字节
│ ... (540行)                             │  每4个Y共享1个V
└─────────────────────────────────────────┘
