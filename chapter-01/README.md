...
<img src="../docs/images/layered-arch.svg" width="100%"/>

| 层级 | 职责 | 核心组件 |
|:---|:---|:---|
| **应用层** | 用户交互、程序入口 | `main.cpp` |
| **调度层** | 数据流控制、生命周期管理 | `SimplePipeline` |
| **核心处理层** | 视频数据处理 | `Demuxer` → `Decoder` → `Renderer` |
| **基础设施** | 可观测性、错误处理 | `Observer`、`Stats`、`ErrorCode` |
