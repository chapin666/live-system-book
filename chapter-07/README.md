...
```mermaid
flowchart TB
    subgraph 应用层["📱 应用层: 帧队列 FrameQueue"]
        L1["作用: 存储解码后的视频帧"]
        L2["数据: 原始像素 (YUV/RGB)"]
        L3["大小: 3-5 帧 (约 100-200ms)"]
        L1 --- L2 --- L3
    end
    
    subgraph 网络层["🌐 网络层: 环形缓冲区 RingBuffer"]
        L4["作用: 存储下载的压缩数据"]
        L5["数据: 压缩码流 (H.264/AAC)"]
        L6["大小: 2-5 秒"]
        L4 --- L5 --- L6
    end
    
    subgraph 传输层["📡 传输层: HTTP 下载线程"]
        L7["作用: 从远程服务器拉取数据"]
        L8["协议: HTTP/1.1, Range 请求"]
        L7 --- L8
    end
    ...
    style 网络层 fill:#e3f2fd,stroke:#1976d2
    style 传输层 fill:#fff3e0,stroke:#f57c00
```
