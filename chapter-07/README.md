...
前六章完成了完整的直播系统：前四章实现观众端播放器，第五章实现采集与 3A 处理，第六章实现编码与推流。在播放端，软件解码在 1080p 场景下尚可接受。在 1080p 场景下，软件解码 CPU 占用 30-50%，尚可接受。但当分辨率提升到 4K（3840x2160）时：
...
第七章将实现**RTMP 直播推流**，完成主播端最后一环：——主播端的开发。
