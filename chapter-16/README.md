...
flowchart TB
    ...
    subgraph 业务逻辑层["🧠 业务逻辑层"]
        B1["状态机管理 StateMachine"]
        B2["错误处理器 ErrorHandler"]
        B3["性能监控器 PerformanceMonitor"]
    end
    
    subgraph Pipeline层["⚙️ Pipeline 层"]
        C1["采集模块 Capture"] --> C2["处理模块 Processor"]
        C2 --> C3["编码模块 Encoder"]
        C3 --> C4["推流模块 Pusher"]
    end
    ...
    style Pipeline层 fill:#e8f5e9
    style 基础能力层 fill:#fce4ec
```
...
```mermaid
flowchart TB
    N0["摄像头/屏幕采集 → 原始帧(YUV/RGB)"]
    N1["美颜处理 (BeautyProcessor) 视频编码 (VideoEncoder) 封装推流 (RtmpPusher)"]
    N2["GPU 磨皮、美白、LUT 调色 H.264/H.265 硬件编码 FLV 封装 + RTMP 推送"]
...
flowchart TB
    N0["麦克风采集 → 原始 PCM 音频帧"]
    N1["3A 处理 (AudioProcessor) 音频编码 (AudioEncoder) 封装推流 (RtmpPusher)"]
    N2["AEC/ANS/AGC AAC 编码 与视频混合封装"]
...
```mermaid
flowchart LR
    N0["采集线程（高优先级） 摄像头采集 ──────→ 帧缓冲队列（无锁） 麦克风采集 ──────→ 音频缓冲队列 处理线程 美颜处理 ←──────── 帧缓冲队列 3A 处理 ←──────── 音频缓冲队列 编码线程 视频编码 ←──────── 处理后帧队列 音频编码 ←──────── 处理后音频队列 推流线程 封装 ←──────── 编码后数据队列 网络发送 ←────── 封装后数据"]
