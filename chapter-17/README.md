...
```mermaid
flowchart TB
    N0["UDP 特点速览 ✓ 无连接 不需要建立连接，直接发送 ✓ 低开销 头部仅 8 字节 ✓ 快速 没有握手和确认延迟 ✗ 不可靠 可能丢包、乱序、重复 ✗ 无流量控制 可能 overwhelming 接收方"]
...
```mermaid
flowchart TB
    N0["数值：0x12345678（32位整数） 大端序（Big Endian）：高位字节在前 地址： 低 ────────────────────> 高 数据： [0x12] [0x34] [0x56] [0x78] 高位 低位 小端序（Little Endian）：低位字节在前 数据： [0x78] [0x56] [0x34] [0x12] 低位 高位"]
