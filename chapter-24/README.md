...
```mermaid
flowchart LR
    N0["SFU 架构 用户 A ──→┌─────────┐←── 用户 B 发送1路 用户 B 特点: N人会议，每人接收 N-1 路流 延迟: 极低 (< 50ms) 适用: 小型会议 (4-20人)"]
    N1["Server (转发) 不解码 不编码"]
    N2["收到1路 (A的) 用户 A 收到1路 (B的)"]
...
```mermaid
flowchart LR
    N0["MCU 架构 用户 A 用户 B 用户 C 用户 D 特点: N人会议，每人只接收 1 路合流 延迟: 较高 (100-500ms) 适用: 大型会议、直播、录制"]
    N1["Server (混音 混画) 解码→合成→编码"]
    N2["所有用户收到 1 路合流 包含 A+B+C"]
...
flowchart LR
    N0["混合架构 转发 CDN"]
    N1["SFU Server MCU HLS/CDN"]
    N2["互动参与者 (10-20人) 低延迟、灵活布局 大规模观众 (1000+人) 接收合流，也可录制成 HLS 仅观看用户"]
...
```mermaid
flowchart TB
    N0["特点：所有参与者平等显示 适用：小型会议、讨论模式 计算：自动计算行列数，均匀分配空间"]
    N1["A D"]
    N2["B E"]
    N3["C F"]
...
```mermaid
flowchart TB
    N0["特点：主讲人占据主要区域，其他人小图显示 适用：讲座、培训、直播 动态：根据语音活动自动切换主讲人"]
    N1["主讲人 (大图 70%) A (小图)"]
    N2["B (小图)"]
    N3["C (小图)"]
...
```mermaid
flowchart TB
    N0["特点：小窗悬浮在主画面上 适用：1v1 通话、小班课 位置：通常右下角，可拖动"]
    N1["主画面 (对方视频)"]
    N2["小窗 (自己画面)"]
...
```mermaid
flowchart LR
    N0["Total Latency: 100-300ms"]
    N1["MCU Pipeline Input → JitterBuffer → Decode → Mixer/Compositor → Encode → Output (RTP) (5-20ms) (10ms) (10-100ms) (30ms) (5ms) Audio ─→ PCM Mixing ───────────────────→ Audio Encode Video ─→ YUV Compositing (GPU) ────────→ Video Encode"]
