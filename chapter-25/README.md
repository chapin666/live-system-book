...
```mermaid
flowchart LR
    N0["SFU 单流录制 用户 A 用户 B 用户 C 录制结果: 多个独立文件，需要后期合成"]
    N1["SFU Server (不解码)"]
    N2["存储 A.webm (用户A的原始流) 存储 B.webm (用户B的原始流)"]
...
```mermaid
flowchart LR
    N0["MCU 合流录制 用户 A 用户 B 用户 C 用户 D 录制结果: 单个文件，可直接播放"]
    N1["MCU Server (解码→合成→编码)"]
    N2["混音混画──→ 存储 meeting.mp4"]
...
```mermaid
flowchart LR
    N0["客户端录制 浏览器/客户端 录制结果: 本地文件，不上传到服务器"]
    N1["getUserMedia ──→ MediaRecorder 存储为 WebM/MP4"]
...
```mermaid
flowchart LR
    N0["编码器输出 ──→ 切片器 ──→ 000.ts ──→ 001.ts ──→ 002.ts 更新 m3u8"]
...
```mermaid
flowchart TB
    N0["Recording Service"]
    N1["Web API (REST) Recording Job Scheduler Output Storage"]
    N2["MP4 Files"]
    N3["Recorder Engine"]
    N4["HLS Segments"]
    N5["Storage Manager"]
    N6["Cloud Storage (S3/OSS/etc)"]
