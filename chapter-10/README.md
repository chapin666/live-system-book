...
```mermaid
flowchart LR
    N0["前五章：播放器端（观众视角） 本地文件播放 网络下载播放 RTMP 直播拉流 硬件解码 第六章起：主播端（主播视角） 音视频采集 ← 本章 音频 3A 处理 ← 第十一章 视频编码 RTMP 推流 连麦互动"]
...
```mermaid
flowchart LR
    N0["直播系统架构 主播端 服务器 观众端"]
    N1["摄像头 麦克风 分发 3A处理 AEC/ANS AGC 观众 1 视频编码 H.264"]
    N2["采集 CDN 分发 推流"]
    N3["接收 转码 观众 2 观众 N"]
    N4["播放器"]
...
```mermaid
flowchart TB
    N0["原始采集数据 时间戳对齐 采集后数据 → 3A处理 → 编码 → 推流"]
    N1["视频采集 摄像头 音频采集 麦克风"]
    N2["1280x720, 30fps, YUV420P 48kHz, 16bit, 立体声"]
...
```mermaid
flowchart TB
    N0["光学镜头 图像传感器 (CMOS/CCD) 光电转换 原始 Bayer 数据 ISP 处理 输出图像 (YUV/RGB/MJPEG)"]
    N1["ISP (Image Signal Processor) - 去噪 (Denoise) - 白平衡 (White Balance) - 曝光补偿 (Exposure) - 色彩校正 (Color Correction) - 锐化 (Sharpen)"]
...
```mermaid
flowchart TB
    N0["理想情况： 视频帧 音频帧 T0 T1 T2 实际情况： 视频帧 ───────────┬────────┬────────┬──────── (延迟 50ms) 音视频不同步！"]
...
```mermaid
flowchart TB
    N0["采集线程 原始帧 处理线程（3A + 编码） 处理后数据 推流线程"]
    N1["帧队列（生产者-消费者）"]
